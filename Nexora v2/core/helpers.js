// v2/core/helpers.js — migrated from 3_core_helpers.js
const weekdayKey = (d) => ['sun','mon','tue','wed','thu','fri','sat'][d.getDay()];
const mmFromHHMM = (s) => { const [h,m] = s.split(':').map(Number); return h*60 + (m || 0); };
const isoAtMinutes = (date, minutes) => { const d = new Date(date); d.setHours(0,0,0,0); return new Date(d.getTime() + minutes * 60000).toISOString(); };
...
export const generateSlotsForDay = ({ clinic, bookings, services, date, serviceId }) => {
  ...
  const sameDay = bookings
    ...
    .map(b => { const d = services.find(s => s.id === b.serviceId)?.durationMin || 30; const start = new Date(b.startsAt).getTime(); return [start, start + d * 60000]; });
  const out = []; const step = 15;
  for (const block of blocks) {
    const [startHH, endHH] = block.split('-');
    const startMin = mmFromHHMM(startHH); const endMin = mmFromHHMM(endHH);
    for (let m = startMin; m <= endMin - durationMin; m += step) {
      const startISO = isoAtMinutes(date, m); const endISO = isoAtMinutes(date, m + durationMin);
      const s = new Date(startISO).getTime(); const e = new Date(endISO).getTime();
