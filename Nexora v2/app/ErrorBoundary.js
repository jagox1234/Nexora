// v2/app/ErrorBoundary.js — migrated from legacy Nexora/0_error_boundary.js
...
export default class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, info){ this.setState({ error, info }); logger.error('ErrorBoundary', error, info); }
  render(){
    ...
    return (
      <ScrollView style={{ flex:1, backgroundColor:'#0b0f13', padding:16 }}>
        ...
        <Text style={{ color:'#93a3af', fontSize:12, lineHeight:18, whiteSpace:'pre-wrap' }}>{stack}</Text>
      </ScrollView>
    );
  }
}
